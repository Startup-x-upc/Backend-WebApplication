...
/**
 * Listener that handles {@link RideCancelledEvent} by clearing the driver's busy status.
 */
@Component("drivermanagementRideCancelledEventListener")
public class RideCancelledEventListener {

    private static final Logger log = LoggerFactory.getLogger(RideCancelledEventListener.class);
    private final DriverRepository driverRepository;

    public RideCancelledEventListener(DriverRepository driverRepository) {
        this.driverRepository = driverRepository;
    }

    @EventListener
    public void onRideCancelled(RideCancelledEvent event) {
        log.info("RideCancelledEvent received in drivermanagement for rideId: {} by driverId: {}.", event.rideId(), event.driverId());
