package org.example.backendwebapplication.shared.interfaces.rest.transform;
 ...
 * Supports i18n error messages via messages.properties.
 */
@NullMarked
public class ErrorResponseAssembler {
    ...
    public static ResponseEntity<ErrorResource> toErrorResponseFromApplicationError(ApplicationError error) {
        HttpStatusCode statusCode = toStatusFromErrorCode(error.code());
    ...
    private static String toSpecificMessageKeyFromErrorCode(String errorCode) {
        return "error.%s.message".formatted(errorCode.toLowerCase(Locale.ROOT));
    }
    ...
    private static String toLocalizedMessageFromApplicationError(ApplicationError error) {
        ...
        String specificMessage = toLocalizedMessageOrNull(
                specificKey, error.details(), toEntityNameFromErrorCode(error.code()));
        ...
        return toLocalizedMessageWithFallback(
                fallbackKey, error.message(),
                error.details(), toEntityNameFromErrorCode(error.code()));
    }
